...
const START_HOUR = 0
const END_HOUR   = 24
const HOUR_H     = 56   // px per hour — 24h grid
...
function snapMin(min, snap = 15) { return Math.round(min / snap) * snap }
function yToMin(y) {
  return Math.max(0, Math.min(END_HOUR * 60 - 30, snapMin(Math.floor((y / HOUR_H) * 60))))
}
function minToY(min) { return (min / 60) * HOUR_H }
function fmtMin(min) {
...
function Modal({ initial, onClose, onSuccess }) {
  ...
  const inp = { background: 'var(--bg)', border: '1px solid var(--border-2)', borderRadius: 8, padding: '9px 13px', color: 'var(--text-1)', fontFamily: 'var(--sans)', fontSize: 14, outline: 'none', width: '100%', boxSizing: 'border-box' }
  const lbl = { fontFamily: 'var(--mono)', fontSize: 10, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--text-3)', marginBottom: 5, display: 'block' }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }} onClick={onClose}>
      <div style={{ background: 'var(--panel)', border: '1px solid var(--border-2)', borderRadius: 20, padding: 28, width: '100%', maxWidth: 440 }} onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {error && <div style={{ color: '#ff6b6b', fontFamily: 'var(--mono)', fontSize: 11, padding: '8px 12px', background: 'rgba(255,80,80,0.08)', borderRadius: 7 }}>{error}</div>}
...
// ── Agent Drawer ──────────────────────────────────────────────────────────────
function AgentDrawer({ open, onClose, fillPct }) {
  const couldFill = Math.min(99, Math.round((100 - fillPct) * 0.78))
  return (
    <>
      {/* Backdrop */}
      {open && <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 90, background: 'rgba(0,0,0,0.35)', backdropFilter: 'blur(3px)' }} />}
      ...
      <div style={{
        ...
        <div style={{
          ...
          boxShadow: '0 32px 80px rgba(0,0,0,0.6), 0 0 0 1px rgba(160,255,121,0.06), inset 0 1px 0 rgba(160,255,121,0.1)',
          display: 'flex', flexDirection: 'column', overflow: 'hidden', padding: 0,
        }}>
          {/* Close */}
          <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '16px 16px 0' }}>
            <button onClick={onClose} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, width: 30, height: 30, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#7E8C7C', cursor: 'pointer', fontSize: 16 }}>×</button>
          </div>

          {/* Content */}
          <div style={{ flex: 1, padding: '8px 24px 24px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 0 }}>

            {/* Badge + avatar */}
            <div style={{ textAlign: 'center', marginBottom: 16 }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(160,255,121,0.12)', border: '1px solid rgba(160,255,121,0.25)', borderRadius: 99, padding: '4px 12px', marginBottom: 14 }}>
                <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#A0FF79' }} />
                <span style={{ fontSize: 11, fontWeight: 700, color: '#A0FF79', letterSpacing: '0.04em' }}>Agente IA · Más popular</span>
              </div>
              <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'linear-gradient(135deg,#2d5a3d,#1a3828)', border: '3px solid rgba(160,255,121,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, margin: '0 auto 12px', boxShadow: '0 0 24px rgba(160,255,121,0.2)' }}>🤖</div>
              <div style={{ fontSize: 30, fontWeight: 900, color: '#F4F7F2', letterSpacing: '-0.03em', lineHeight: 1, marginBottom: 4 }}>Mateo</div>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#A0FF79', letterSpacing: '0.12em', textTransform: 'uppercase' }}>Especialista en ocupación</div>
            </div>

            {/* Quote */}
            <div style={{ background: 'rgba(160,255,121,0.06)', border: '1px solid rgba(160,255,121,0.12)', borderRadius: 12, padding: '12px 16px', marginBottom: 16, textAlign: 'center' }}>
              <span style={{ fontStyle: 'italic', color: '#B6C4B2', fontSize: 14, lineHeight: 1.5 }}>"Tu agenda siempre llena."</span>
            </div>

            {/* Description */}
            <p style={{ margin: '0 0 16px', fontSize: 13.5, color: '#7E8C7C', lineHeight: 1.65, textAlign: 'center' }}>
              Mateo detecta los huecos libres antes de que se pierdan. Localiza clientes anteriores y los contacta con el mensaje justo en el momento oportuno, convirtiendo tiempo muerto en ingresos.
            </p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
              {[
                ...
                <div key={f} style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  <div style={{ width: 18, height: 18, borderRadius: '50%', background: 'rgba(160,255,121,0.1)', border: '1px solid rgba(160,255,121,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 1 }}>
                    <div style={{ width: 5, height: 5, borderRadius: '50%', background: '#A0FF79' }} />
                  </div>
                  <span style={{ fontSize: 13, color: '#B6C4B2', lineHeight: 1.4 }}>{f}</span>
                </div>
              ))}
            </div>

            {/* Big stat */}
            <div style={{ background: 'linear-gradient(135deg, rgba(160,255,121,0.12), rgba(160,255,121,0.05))', border: '1px solid rgba(160,255,121,0.2)', borderRadius: 16, padding: '20px', marginBottom: 20, textAlign: 'center', boxShadow: '0 0 40px rgba(160,255,121,0.08) inset' }}>
              <div style={{ fontSize: 52, fontWeight: 900, color: '#A0FF79', lineHeight: 1, letterSpacing: '-0.04em', textShadow: '0 0 32px rgba(160,255,121,0.5)' }}>+{couldFill}%</div>
              <div style={{ fontSize: 12, color: '#7E8C7C', marginTop: 6, fontWeight: 500 }}>de tu agenda podría llenar</div>
            </div>

            {/* Price + CTA */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              <div style={{ flexShrink: 0 }}>
                <div style={{ fontSize: 26, fontWeight: 900, color: '#F4F7F2', lineHeight: 1, letterSpacing: '-0.02em' }}>79 €</div>
                <div style={{ fontSize: 11, color: '#5A7060', marginTop: 2 }}>al mes</div>
              </div>
              <button style={{ flex: 1, background: 'var(--green)', color: '#0B0E0C', border: 'none', borderRadius: 12, padding: '14px', fontFamily: 'var(--sans)', fontSize: 15, fontWeight: 800, cursor: 'pointer', letterSpacing: '-0.01em', boxShadow: '0 8px 24px rgba(160,255,121,0.3)', transition: 'transform 0.1s, box-shadow 0.1s' }}
...
export default function ReservasPage() {
  ...
  // drag = { dateStr, startMin, endMin } | null
  ...
  // Load reservas for the current week
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: 'var(--bg)', fontFamily: 'var(--sans)' }}>
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        ...
        {/* ── Header ── */}
        <header style={{ flexShrink: 0, padding: '14px 20px 12px', borderBottom: '1px solid var(--border)', background: 'var(--panel)', display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
          ...
          {/* Fill % */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'var(--elevated)', border: '1px solid var(--border-2)', borderRadius: 12, padding: '9px 14px' }}>
            <div>
              <div style={{ fontFamily: 'var(--mono)', fontSize: 9, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--text-3)', marginBottom: 4 }}>Semana</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{ width: 100, height: 5, background: 'var(--border-2)', borderRadius: 99, overflow: 'hidden' }}>
            ...
            <div>
              <div style={{ fontFamily: 'var(--mono)', fontSize: 9, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--text-3)', marginBottom: 2 }}>Reservas</div>
              <div style={{ fontFamily: 'var(--mono)', fontSize: 14, fontWeight: 800, color: 'var(--text-1)' }}>{bookedSlots}</div>
            </div>
          </div>

          {/* Mateo CTA button */}
          <button onClick={() => setAgentOpen(true)}
            style={{ background: 'linear-gradient(135deg,#1a3828,#0d1f11)', border: '1px solid rgba(160,255,121,0.3)', borderRadius: 10, padding: '8px 14px', color: '#A0FF79', fontFamily: 'var(--sans)', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7, boxShadow: '0 4px 16px rgba(160,255,121,0.1)', flexShrink: 0 }}>
            🤖 <span>Contratar Mateo</span>
          </button>
            ...
            {weekDays.map(day => {
              ...
              return (
                <div key={toDateStr(day)} style={{ flex: 1, padding: '10px 8px 8px', textAlign: 'center', borderLeft: '1px solid var(--border)', background: isToday ? 'rgba(160,255,121,0.04)' : 'transparent' }}>
                  <div style={{ fontFamily: 'var(--mono)', fontSize: 9, letterSpacing: '0.12em', textTransform: 'uppercase', color: isToday ? 'var(--green)' : 'var(--text-3)', marginBottom: 4 }}>{DAYS_ES[day.getDay()]}</div>
                  <div style={{ width: 28, height: 28, borderRadius: '50%', background: isToday ? 'var(--green)' : 'transparent', color: isToday ? 'var(--bg)' : 'var(--text-1)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto', fontWeight: isToday ? 800 : 600, fontSize: 14 }}>{day.getDate()}</div>
                </div>
              )
            })}
          </div>
            ...
            <div style={{ display: 'flex', minHeight: GRID_H }}>
              ...
              <div style={{ width: TIME_COL_W, flexShrink: 0, position: 'relative' }}>
                {Array.from({ length: TOTAL_H + 1 }, (_, i) => (
                  <div key={i} style={{ position: 'absolute', top: i * HOUR_H - 8, right: 8, fontFamily: 'var(--mono)', fontSize: 9.5, color: 'var(--text-3)', textAlign: 'right', userSelect: 'none', letterSpacing: '0.03em' }}>
              ...
              {weekDays.map(day => {
                ...
                return (
                  <div key={dateStr}
                    ...
                    style={{ flex: 1, borderLeft: '1px solid var(--border)', position: 'relative', cursor: 'crosshair', background: isToday ? 'rgba(160,255,121,0.015)' : 'transparent' }}
                  >
                    ...
                    {isToday && nowY >= 0 && nowY <= GRID_H && (
                      <div style={{ position: 'absolute', top: nowY, left: -5, right: 0, zIndex: 10, pointerEvents: 'none' }}>
                        <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#ff3b3b', position: 'absolute', top: -4 }} />
                        <div style={{ height: 2, background: '#ff3b3b', marginLeft: 6, boxShadow: '0 0 8px rgba(255,59,59,0.6)' }} />
                      </div>
                    )}
                    ...
                    {isDragging && drag && (
                      <div style={{
                        ...
                        borderRadius: 8,
                        background: 'rgba(160,255,121,0.2)',
                        border: '2px solid var(--green)',
                        backdropFilter: 'blur(2px)',
                      }}>
                        <div style={{ padding: '4px 8px', fontFamily: 'var(--mono)', fontSize: 9, color: 'var(--green)', fontWeight: 700 }}>
                    ...
                    {/* Reservas */}
                    {dayReservas.map(r => {
                      const top = minToY(parseInt(r.hora) * 60 + parseInt((r.hora || '00:00').split(':')[1] || 0))
                      const cfg = STATUS_CFG[r.estado] || STATUS_CFG.pendiente
                      return (
                        <div key={r.id}
                          onMouseDown={e => e.stopPropagation()}
                          style={{ position: 'absolute', top: top + 1, left: 3, right: 3, height: HOUR_H - 4, borderRadius: 8, background: cfg.bg, border: `1.5px solid ${cfg.border}`, padding: '4px 7px', overflow: 'hidden', cursor: 'default', zIndex: 5 }}>
      ...
      {/* Agent drawer */}
      <AgentDrawer open={agentOpen} onClose={() => setAgentOpen(false)} fillPct={fillPct} />
      ...
      {/* Toast */}
      {toast && (
        <div style={{ position: 'fixed', bottom: 28, right: 28, zIndex: 300, background: 'rgba(160,255,121,0.12)', border: '1px solid rgba(160,255,121,0.3)', borderRadius: 12, padding: '12px 20px', display: 'flex', alignItems: 'center', gap: 10, backdropFilter: 'blur(12px)', boxShadow: '0 8px 32px rgba(0,0,0,0.4)' }}>
